...
def xavier_uniform(fan_in: int, fan_out: int, gain: float = 1.0, **kwargs: Any) -> "Tensor":
    ...
    return rand(fan_in, fan_out, low=-a, high=a, requires_grad=True, **kwargs)
    ### END YOUR SOLUTION
...
def xavier_normal(fan_in: int, fan_out: int, gain: float = 1.0, **kwargs: Any) -> "Tensor":
    ...
    return randn(fan_in, fan_out, mean=0, std=std, requires_grad=True, **kwargs)
    ### END YOUR SOLUTION
...
def kaiming_uniform(fan_in: int, fan_out: int, nonlinearity: str = "relu", **kwargs: Any) -> "Tensor":
    ...
    return rand(fan_in, fan_out, low=-bound, high=bound, requires_grad=True, **kwargs)
    ### END YOUR SOLUTION
