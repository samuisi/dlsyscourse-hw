...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  /// BEGIN SOLUTION
  size_t tmp = gid;
  for (int i = shape.size-1; i >= 0 && tmp; i--) {
    size_t index = tmp % shape.data[i];
    offset += index * strides.data[i];
    tmp /= shape.data[i];
  }
  out[gid] = a[offset];
  /// END SOLUTION
...
__global__ void EwiseLogKernel(const scalar_t* a, size_t size, scalar_t* out) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
...
void EwiseLog(const CudaArray& a, CudaArray* out) {
  CudaDims dim = CudaOneDim(out->size);
  EwiseLogKernel<<<dim.grid, dim.block>>>(a.ptr, a.size, out->ptr);
}

__global__ void EwiseExpKernel(const scalar_t* a, size_t size, scalar_t* out) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
...
void EwiseExp(const CudaArray& a, CudaArray* out) {
  CudaDims dim = CudaOneDim(out->size);
  EwiseExpKernel<<<dim.grid, dim.block>>>(a.ptr, a.size, out->ptr);
}

__global__ void EwiseTabhKernel(const scalar_t* a, size_t size, scalar_t* out) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
...
void EwiseTanh(const CudaArray& a, CudaArray* out) {
  CudaDims dim = CudaOneDim(out->size);
  EwiseTabhKernel<<<dim.grid, dim.block>>>(a.ptr, a.size, out->ptr);
}
...
__global__ void ReduceMaxKernel(const scalar_t* a, size_t size, scalar_t* out, size_t reduce_size) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  
  if (gid >= size / reduce_size) return;
  size_t base = gid * reduce_size;
  scalar_t mx = a[base];
  for (size_t i = 1; i < reduce_size; i++) mx = mx > a[base+i] ? mx : a[base+i];
  out[gid] = mx;
}
...
void ReduceMax(const CudaArray& a, CudaArray* out, size_t reduce_size) {
  ...
  ReduceMaxKernel<<<dim.grid, dim.block>>>(a.ptr, a.size, out->ptr, reduce_size);
  /// END SOLUTION
}

__global__ void ReduceSumKernel(const scalar_t* a, size_t size, scalar_t* out, size_t reduce_size) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  
  if (gid >= size / reduce_size) return;
  size_t base = gid * reduce_size;
  scalar_t sum = 0;
  for (size_t i = 0; i < reduce_size; i++) sum += a[base + i];
  out[gid] = sum;
}
...
void ReduceSum(const CudaArray& a, CudaArray* out, size_t reduce_size) {
  ...
  /// BEGIN SOLUTION
  CudaDims dim = CudaOneDim(a.size / reduce_size);
  ReduceSumKernel<<<dim.grid, dim.block>>>(a.ptr, a.size, out->ptr, reduce_size);
  /// END SOLUTION
}
